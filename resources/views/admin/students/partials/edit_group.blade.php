...
<form method="POST" action="{{ route("admin.students.update_group") }}" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    ...
    <div class="row"> 
        <div class="form-group col-md-6">
            ...
            <input class="form-control {{ $errors->has('transfer_name') ? 'is-invalid' : '' }}" type="text" name="transfer_name" id="transfer_name" value="{{ old('transfer_name', $group->pivot->transfer_name) }}" >
        ...
        <div class="form-group col-md-6">
            ...
            <input class="form-control {{ $errors->has('reference_number') ? 'is-invalid' : '' }}" type="text" name="reference_number" id="reference_number" value="{{ old('reference_number', $group->pivot->reference_number) }}" >
        ...
        <div class="form-group col-md-4">
            ...
            @foreach(App\Models\Group::PAYMENT_TYPE_SELECT as $key => $label)
                <div class="form-check {{ $errors->has('payment_type') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="radio" id="payment_type_{{ $key }}" name="payment_type" value="{{ $key }}" {{ old('payment_type', $group->pivot->payment_type) === (string) $key ? 'checked' : '' }} >
        ...
        <div class="form-group col-md-4">
            ...
            @foreach(App\Models\Group::PAYMENT_STATUS_SELECT as $key => $label)
                <div class="form-check {{ $errors->has('payment_status') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="radio" id="payment_status_{{ $key }}" name="payment_status" value="{{ $key }}" {{ old('payment_status', $group->pivot->payment_status) === (string) $key ? 'checked' : '' }} >
        ...
        <div class="form-group col-md-4">
            ...
            @foreach(App\Models\Group::STATUS_SELECT as $key => $label)
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $group->pivot->status) === (string) $key ? 'checked' : '' }} required>
