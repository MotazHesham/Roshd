...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.centerServicesPackage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.center-services-packages.update", [$centerServicesPackage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row">
                <div class="form-group col-md-3">
                    <label class="required" for="name">{{ trans('cruds.centerServicesPackage.fields.name') }}</label>
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $centerServicesPackage->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group col-md-3">
                    ...
                    <input class="form-control {{ $errors->has('package_value') ? 'is-invalid' : '' }}" type="number" name="package_value" id="package_value" value="{{ old('package_value', $centerServicesPackage->package_value) }}" step="1" required>
                ...
                <div class="form-group col-md-3">
                    ...
                    <input class="form-control {{ $errors->has('sessions') ? 'is-invalid' : '' }}" type="number" name="sessions" id="sessions" value="{{ old('sessions', $centerServicesPackage->sessions) }}" step="1" required>
                ...
                <div class="form-group col-md-3">
                    ...
                    <input class="form-control {{ $errors->has('free_sessions') ? 'is-invalid' : '' }}" type="number" name="free_sessions" id="free_sessions" value="{{ old('free_sessions', $centerServicesPackage->free_sessions) }}" step="1" required>
                ...
                <div class="form-group col-md-3">
                    ...
                    @foreach(App\Models\CenterServicesPackage::ATTEND_TYPE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('attend_type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="attend_type_{{ $key }}" name="attend_type" value="{{ $key }}" {{ old('attend_type', $centerServicesPackage->attend_type) === (string) $key ? 'checked' : '' }} required>
                ...
                <div class="form-group col-md-3">
                    ...
                    @foreach(App\Models\CenterServicesPackage::DOCTOR_TYPE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('doctor_type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="doctor_type_{{ $key }}" name="doctor_type" value="{{ $key }}" {{ old('doctor_type', $centerServicesPackage->doctor_type) === (string) $key ? 'checked' : '' }} required>
                ...
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control {{ $errors->has('package_content') ? 'is-invalid' : '' }}" name="package_content" id="package_content">{{ old('package_content', $centerServicesPackage->package_content) }}</textarea>
